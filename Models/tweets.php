...
function findTweets(array $user, string $keyword = null, array $user_ids = null)
{
    ...
    $query = <<<SQL
        ...
        FROM
            tweets AS T
            -- ツイートテーブルを別名Tにする
            -- JOINでツイートテーブルとユーザーテーブルを紐付け、状態がactiveなものを表示
            JOIN
    ...
    if(isset($keyword)) {
        ...
        // ツイート主のニックネーム・・ユーザー名・本文から・本文から部分一致・本文から部分一致検索
        // CONCAT() は引数の文字を連結
    ...
    $mysqli->close(); //もしクローズしなかったらPHPの一連の処理が終わって終わってから自動で閉じる
